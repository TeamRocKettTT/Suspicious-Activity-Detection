...
X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.33) #split dataset into train and test
...
model.add(TimeDistributed(MaxPooling2D((4, 4))))
model.add(TimeDistributed(Conv2D(128, (3, 3), padding='same',activation = 'softmax')))
...
if os.path.exists("model/cnn_weights.hdf5") == False:
    ...
    hist = model.fit(X_train, y_train, batch_size = 32, epochs = 7, validation_data=(X_test, y_test), callbacks=[model_check_point], verbose=1)
    ...
    if request.method == 'POST':
        ...
        while True:
            ...
            if ret == True:
                ...
                score = ( np.mean(predict) + np.amax(predict) ) / 2
                print(str(pred)+" "+str(score))
                frame = cv2.resize(frame, (500, 500)) 
                if score > 0.10 and option == 0:
                    option = 1
